...
comments: true
toc: true
pageviews__total: 42762
pageviews__recent:  233
pageviews__avg_time: 310
tutorial__order: 0
...
Node modules allow you to write reusable code. You can include your own one into another module. Using Node Package Manager (NPM), you can publish your module to the community. Also, NPM enables you to reuse modules made by other developers.
...
As you can see, we imported the "fs" module into our system. It allows us to (what?) any function attached to it, like "readFile".
...
`exports` are used to generate modules. It gives you the chance to export your own objects and functions. Let's do an example:
...
In the code below, we are exporting the `area` function. We defined the constant PI but this is only accessible within the module. Only the elements associated with `exports` are available outside the module.
...
Module is not global, it is local for each module. It contains metadata about a module id, exports, parent, children, ...
...
2. If you prefer to return a single object that exposes multiple assignments. e.g.`module.exports = {area: area, circumference: circunference};`


# Imports

Imports are not accessible in Node as the version 6. However, it might come in future versions.
...
You can try it today, using transpilers such as Traceur Compiler, Babel or Rollup. But, that will be for another post.
